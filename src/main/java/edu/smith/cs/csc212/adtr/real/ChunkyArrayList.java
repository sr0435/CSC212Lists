...
public class ChunkyArrayList<T> extends ListADT<T> {
	...
	public void addIndex(int index, T item) {
		...
		for (FixedSizeList<T> chunk : this.chunks) {
			...
			// if the index is the same size as the chunk, then we need it to move to the next chunk
			// so it needs to be less than or equal to end-1
			// Phuong helped me write this if statement
